...
class SettingsController extends Controller
{
    ...
    public function borrow_reason_page(){
        return view('pages.pages_backend.settings.settings_borrow_reason');
    }

    public function borrow_reason_data(){
       $DataBorrowOption = DataBorrowOption::latest('updated_at')
            ->whereNull('deleted_at')
            ->get();

        return DataTables::of($DataBorrowOption)
            ->setRowId('id')
            ->make(true);
    }

    public function borrow_reason_get($id){
        $DataBorrowOption = DataBorrowOption::findOrFail($id);
        return response()->json($DataBorrowOption);
    }

    public function borrow_reason_create(Request $request){
        DataBorrowOption::create([
            'reason' => $request->name,
            'status' => 'Active',
            'created_at' => Carbon::now(),
            'updated_at' => Carbon::now(),
    ...
    public function borrow_reason_update(Request $request){
        $DataBorrowOption = DataBorrowOption::findOrFail($request->item_id);
        $DataBorrowOption->update([  // Update the instance instead of using the class method
            'reason' => $request->name,
            'status' => $request->status,
            'updated_at' => Carbon::now(),
        ]);
        ...
        SystemLogs::create([
            ...
            'description_logs' => [
                ...
                'old_details' => [
                    'Old Borrow Reason' => $DataBorrowOption->reason,
                    'Old Status' => $DataBorrowOption->status,
                ],
            ],
            'employee_id' => Auth::user()->employee_id,
            'ip_address' => $request->ip(),
